...
- **Network members, countries, total chemical mixes tried, growth chart** —
  all computed live from real signups/mix-checker usage once Firebase is
  connected. No seeded or fake numbers are baked into the code. Sign-up only
  asks for a name and country (no email) to keep friction low and avoid
  collecting contact data you don't need.
- **Verified reference database / chemical connection network** — 68
  substance pairs (including bleach, ammonia, hydrogen peroxide, sulfuric
  acid, sodium cyanide, white phosphorus, and more) are written by you as
  the curated baseline, sourced conceptually from CAMEO Chemicals (NOAA),
  CDC/NIOSH, OSHA, NFPA 491, NLM HSDB, EPA, IARC, and WHO public hazard
  classifications. Cite real sources if you use this for a competition or
  application — don't represent the existing entries as pulled live from an
  official API unless you wire one in.
- The site includes a permanent disclaimer with the real US Poison Control
  number (1-800-222-1222) and a note to use local equivalents outside the
  USA — keep this intact in any version you ship.
